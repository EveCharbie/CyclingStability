...
class Simulator:
    ...
    def __init__(self, system: System) -> None:
        if not isinstance(system, System):
            raise TypeError(f"System should be of type {type(System)} not {type(system)}.")
        self._system = system
        self._constants = {}
    ...
    def constants(self, constants: dict[Basic, float]) -> None:
        if not isinstance(constants, dict):
            raise TypeError(f"Constants should be of type {type(dict)} not " f"{type(constants)}.")
        ...
        if self._initialized:
            if set(old_constants.keys()) != set(constants.keys()):
                ...
                self._p_vals = np.array([constants[pi] for pi in self._p], dtype=np.float64)
    ...
    @inputs.setter
    def inputs(self, inputs: dict[Function, Callable[[float, array_type], float]]) -> None:
        if not isinstance(inputs, dict):
            raise TypeError(f"Inputs should be of type {type(dict)} not " f"{type(inputs)}.")
        for ri in inputs.values():
            if not callable(ri):
                raise TypeError(
                    f"Inputs should be of type " f"{type(Callable[[float, array_type], float])} not {type(ri)}."
                )
    ...
    def initial_conditions(self, initial_conditions: dict[Function, float]) -> None:
        if not isinstance(initial_conditions, dict):
            raise TypeError(f"Initial condintions should be of type " f"{type(dict)} not {type(initial_conditions)}.")
    ...
    def _eval_eoms_reshaped(self, t: float, x: array_type) -> tuple[array_type, array_type]:
        """Evaluate the equations of motion and reshape the output."""
        mass_matrix, forcing = self._eval_eoms_matrices(
            t, x, self._p_vals, np.array([cf(t, x) for cf in self._r_funcs], dtype=np.float64)
        )
        return (mass_matrix.reshape((self._n_x, self._n_x)), forcing.reshape((self._n_x,)))

    def _solve_configuration_constraints(self, q_ind: array_type, q_dep_guess: array_type) -> array_type:
        """Solve the configuration constraints for the dependent coordinates."""
        if not self.system.q_dep:
            return np.array([])
        return np.array(fsolve(self._eval_configuration_constraints, q_dep_guess, args=(q_ind, self._p_vals)))

    def _solve_velocity_constraints(self, q: array_type, u_ind: array_type, u_dep_guess: array_type) -> array_type:
        """Solve the velocity constraints for the dependent speeds."""
        if not self.system.u_dep:
            return np.array([])
        return np.array(fsolve(self._eval_velocity_constraints, u_dep_guess, args=(q, u_ind, self._p_vals)))

    def solve_initial_conditions(self) -> None:
        """Solve the initial conditions for the dependent coordinates and speeds."""
        if self._eval_configuration_constraints is None or self._eval_velocity_constraints is None:
            raise ValueError("Simulator has not been initialized yet.")
        q_dep_guess = [self.initial_conditions.get(qi, 0.0) for qi in self.system.q_dep]
        q_ind = [self.initial_conditions[qi] for qi in self.system.q_ind]
        q_dep = self._solve_configuration_constraints(np.array(q_ind), np.array(q_dep_guess))
        q0 = q_ind + list(q_dep)
        u_dep_guess = [self.initial_conditions.get(ui, 0.0) for ui in self.system.u_dep]
        u_ind = [self.initial_conditions[ui] for ui in self.system.u_ind]
        u_dep = self._solve_velocity_constraints(np.array(q0), np.array(u_ind), np.array(u_dep_guess))
        u0 = u_ind + list(u_dep)
    ...
    def initialize(self, check_parameters: bool = False) -> None:
        ...
        if check_parameters:
            free_constants = self.system.mass_matrix_full.free_symbols.union(self.system.forcing_full.free_symbols)
            ...
            free_symbols = free_constants.union(free_dyn).difference({dynamicsymbols._t})
            missing = free_symbols.difference(self.constants.keys())
            if missing:
                raise ValueError(f"Simulator is missing the following constants: " f"{missing}.")
            missing = set(self.system.q_ind.col_join(self.system.u_ind)).difference(self.initial_conditions.keys())
            if missing:
                raise ValueError(f"Simulator is missing the following initial " f"conditions: {missing}.")

        qdot_to_u = self.system.eom_method.kindiffdict() if isinstance(self.system.eom_method, KanesMethod) else {}
        t = dynamicsymbols._t
        ...
        if self.inputs:
            ...
            self._r, self._r_funcs = (), ()
        velocity_constraints = msubs(
            self.system.holonomic_constraints.diff(t).col_join(self.system.nonholonomic_constraints), qdot_to_u
        )
        self._eval_configuration_constraints = lambdify(
            (self.system.q_dep, self.system.q_ind, self._p), self.system.holonomic_constraints[:], cse=True
        )
        ...
        self._eval_eoms_matrices = lambdify(
            ...
            (
                ...
                self.system.forcing_full.reshape(1, self._n_x),
            ),
            cse=True,
        )
        self.solve_initial_conditions()
    ...
    def _eval_eoms(self, t: float, x: array_type, xd: array_type, residual: array_type) -> None:
        ...
        if self._n_qdep != 0:
            residual[self._n_x - self._n_udep : -n_nh] = self._eval_configuration_constraints(
                q_dep, q_ind, self._p_vals
            )
        if n_nh != 0:
            residual[-n_nh:] = self._eval_velocity_constraints(u_dep, q, u_ind, self._p_vals)[-n_nh:]
    ...
    def solve(
        ...
        **kwargs
        ...
        x0 = np.array([self.initial_conditions[xi] for xi in self.system.q.col_join(self.system.u)])
        if solver == "solve_ivp":
            ...
            n_constrs = len(self.system.holonomic_constraints) + len(self.system.nonholonomic_constraints)
            dae_solver = dae(
                integrator_name,
                self._eval_eoms,
                algebraic_vars_idx=range(len(x0) - n_constrs, len(x0)),
                old_api=False,
                **kwargs,
            )
